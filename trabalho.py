...
def deletar_produto():
    global doces, preco  # Mover para o topo da função
    ...
    listar()
    try:
        posicao = int(input("Digite a posição do doce para deletar: ")) - 1
    except ValueError:
        print("Digite apenas números!")
        return

    if 0 <= posicao < len(doces):
        nome_removido = doces[posicao]
        
        doces = doces[:posicao] + doces[posicao+1:]
        preco = preco[:posicao] + preco[posicao+1:]

        print(f"{nome_removido} foi deletado com sucesso.")
    else:
